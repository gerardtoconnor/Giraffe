...
type XmlNode<'T> =
    | ParentNode  of XmlElement * XmlNode<'T> list // An XML element which contains nested XML elements
    ...
    | Binding     of ('T -> string)
    | BindIf      of ('T -> bool) * XmlNode<'T> * XmlNode<'T>
    
type XmlNodeParent<'T1,'T2> =
    | Node of XmlNode<'T1>
    | BindFor of ('T1 -> 'T2 seq) * XmlNode<'T1>

type XmlNodeParent<'T1,'T2,'T3> =
    | Node of XmlNode<'T1>
    | BindFor of ('T1 -> 'T2 seq) * XmlNodeParent<'T2,'T3>

type XmlNodeParent<'T1,'T2,'T3,'T4> =
    | Node of XmlNode<'T1>
    | BindFor of ('T1 -> 'T2 seq) * XmlNodeParent<'T2,'T3,'T4>

type XmlNodeParent<'T1,'T2,'T3,'T4,'T5> =
    | Node of XmlNode<'T1>
    | BindFor of ('T1 -> 'T2 seq) * XmlNodeParent<'T2,'T3,'T4,'T5>

type XmlNodeParent<'T1,'T2,'T3,'T4,'T5,'T6> =
    | Node of XmlNode<'T1>
    | BindFor of ('T1 -> 'T2 seq) * XmlNodeParent<'T2,'T3,'T4,'T5,'T6>

let inline parentNode (attr:XmlAttribute) (children:'T) = 
...
type XmlNode<'T,'U> =
    ...
    | Binding     of ('T -> string)
    | BindIf      of ('T -> bool) * XmlNode<'T> * XmlNode<'T>
    | BindFor     of ('T -> 'U seq) * XmlNode<'U>

type XmlNode<'T,'T1,'T2> =
    | ParentNode  of XmlElement * XmlNode<'T> list // An XML element which contains nested XML elements
    | VoidElement of XmlElement                // An XML element which cannot contain nested XML (e.g. <hr /> or <br />)
    | EncodedText of string                    // XML encoded text content
    | RawText     of string                    // Raw text content
    | Binding     of ('T -> string)
    | BindIf      of ('T -> bool) * XmlNode<'T> * XmlNode<'T>
    | BindFor     of ('T -> 'T1 seq) * XmlNode<'T1,'T2>
